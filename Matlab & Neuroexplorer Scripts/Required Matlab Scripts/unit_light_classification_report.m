...
report{1,3}='spike_p_sig';
report{1,4} = 'wave_p_sig';

report{1,5}='spike_p_val';
...
for Q=1:length(DATA)
    for u=1:length(DATA(Q).units)
        ...
        report{row,3}= spike_p<.05;

        report{row,5}=spike_p;%'PreVsPost_pulse_spikes_p_val';
        report{row,6}=DATA(Q).units(u).lightResponse.tests.PrePost.PrePostChange;%'PreVsPost_pulse_spikes_rate_change';
