%% The first half of this script was separated and saved as make_RATES_structure_and_table.
% keyboard
...
clearvars comb* curr*
% dbstop if error
% fprintf('Paused, to continue type dbcont\n')
% keyboard
%% calculate Z-scores
...
PLOT_CRFvsNonCRF(sCRF).dataType='CRF';
PLOT_CRFvsNonCRF(sCRF).sortedIdx=sortedUnitsIdx;
PLOT_CRFvsNonCRF(sCRF).sortedRateData=sortedRateData;
PLOT_CRFvsNonCRF(sCRF).sortedBurstData=sortedBurstData;
...
sortedBurstData=[currBurstData([resIdx],:)];
sortedUnitsIdx=[currUnitIdx([resIdx],:)];
PLOT_CRFvsNonCRF(2).dataType='NonCRF';
PLOT_CRFvsNonCRF(2).sortedIdx=sortedUnitsIdx;
PLOT_CRFvsNonCRF(2).sortedRateData=sortedRateData;
PLOT_CRFvsNonCRF(2).sortedBurstData=sortedBurstData;

% end
%% PLOT_CRFvsNonCRF, now create and graph PLOTdata
close all
fprintf('Plotting data from PLOT_CRFvsNonCRF.sortedRateData.\n')
spaceRow=NaN(1,size(PLOT_CRFvsNonCRF(1).sortedRateData,2));
rateData=[PLOT_CRFvsNonCRF(1).sortedRateData;spaceRow;PLOT_CRFvsNonCRF(2).sortedRateData];
...
fprintf('\n\n To make normalized firing plot for CRF units by lick type\nrun plot_normalized_firing_CRF_by_lick_types.m')
fprintf('\nIf you have not yet exported results to excel file (name=Unit Results Summary Table.xlsx)\nrun print_results_table_to_excel.m\n')
